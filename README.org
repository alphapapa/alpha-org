...
5. Select the tree sidebar window and press =<S-tab>= to globally cycle the tree's outline visibility.
6. Click on a heading *(which one?)* in the tree sidebar to show that entry's contents.
